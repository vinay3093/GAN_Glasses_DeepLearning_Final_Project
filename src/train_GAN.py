...
with tf.Session(config=tfConfig) as sess:
	...
	if opt.fromIt!=0:
		...
		print("resuming from iteration {0}...".format(opt.fromIt))
	elif opt.warpN>1:
		...
		print("loading pretrained GP ({0})...".format(opt.loadGP))
		util.restoreModelPrevStage(opt,sess,saver_D,"D")
		print("continue to train D...")
	else:
		if opt.loadGP:
			util.restoreModel(opt,sess,saver_GPcur,opt.loadGP,"GP")
			print("loading pretrained GP ({0})...".format(opt.loadGP))
		if opt.loadD:
			...
			print(("loading pretrained D ({0})...".format(opt.loadD))
	...
	for i in range(opt.fromIt,opt.toIt):
		...
		if (i+1)%2000==0:
			...
			print("model saved: {0}/{1}, it.{2}".format(opt.group,opt.name,i+1))

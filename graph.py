...
def geometric_multires(opt,imageBG,imageFG,p):
	...
	with tf.compat.v1.variable_scope("geometric"):
		imageFGwarpAll,pAll = [],[p]
		dp = None
		# define spatial transformations
		for l in range(opt.warpN):
			with tf.compat.v1.variable_scope("warp{0}".format(l)):
				...
				with tf.compat.v1.variable_scope("conv1"): feat,imageConcat = conv2Layer(opt,feat,imageConcat,32) # 72x72
				with tf.compat.v1.variable_scope("conv2"): feat,imageConcat = conv2Layer(opt,feat,imageConcat,64) # 36x36
				with tf.compat.v1.variable_scope("conv3"): feat,imageConcat = conv2Layer(opt,feat,imageConcat,128) # 18x18
				with tf.compat.v1.variable_scope("conv4"): feat,imageConcat = conv2Layer(opt,feat,imageConcat,256) # 9x9
				with tf.compat.v1.variable_scope("conv5"): feat,imageConcat = conv2Layer(opt,feat,imageConcat,512) # 5x5
				feat = tf.reshape(feat,[opt.batchSize,-1])
				with tf.compat.v1.variable_scope("fc6"): feat = linearLayer(opt,feat,256)
				with tf.compat.v1.variable_scope("fc7"): feat = linearLayer(opt,feat,opt.warpDim,final=True)
				dp = feat
...
def discriminator(opt,image,reuse=False):
	...
	def conv2LayerScore(opt,feat,outDim):
		...
		return conv
	with tf.compat.v1.variable_scope("discrim",reuse=reuse):
		feat = image # 144x144
		with tf.compat.v1.variable_scope("conv1"): feat = conv2Layer(opt,feat,32) # 72x72
		with tf.compat.v1.variable_scope("conv2"): feat = conv2Layer(opt,feat,64) # 36x36
		with tf.compat.v1.variable_scope("conv3"): feat = conv2Layer(opt,feat,128) # 18x18
		with tf.compat.v1.variable_scope("conv4"): feat = conv2Layer(opt,feat,256) # 9x9
		with tf.compat.v1.variable_scope("conv5"): feat = conv2Layer(opt,feat,512) # 5x5
		with tf.compat.v1.variable_scope("conv6"): feat = conv2LayerScore(opt,feat,1) # 5x5
		score = feat
	return score
...
def createVariable(opt,weightShape,biasShape=None,stddev=None):
	...
	weight = tf.compat.v1.get_variable("weight",shape=weightShape,dtype=tf.float32,
									  initializer=tf.random_normal_initializer(stddev=stddev))
	bias = tf.compat.v1.get_variable("bias",shape=biasShape,dtype=tf.float32,
